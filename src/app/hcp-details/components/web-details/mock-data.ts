  ...
  */
  const payload: any = {
    ...
    data: {
      ...
      indicator: [
        { max: 10, min: 0, name: '学术影响力', color: '#000000' },
        { max: 10, min: 0, name: '治疗理念', color: '#000000' },
        { max: 10, min: 0, name: '产品经验', color: '#000000' },
        { max: 10, min: 0, name: '患者管理', color: '#000000' },
        { max: 10, min: 0, name: '区域带教', color: '#000000' },
      ],
    },
  };
  ...
  let option: any = {
    ...
    tooltip: {
      ...
      formatter: (data: any) => {
        ...
        console.log("tip===", tmpData);
        data.data.forEach((item: any, index: number) => {
          tip += data.marker + ' ' + payload.data.x[index] + '：' + tmpData[index] + '分<br/>'
        })
        return tip
      }
    },
...
1	无公开学术活动，未见发表或会议参与
2	偶尔参与本地学术会议，无署名论文
3	在省级学会有任职，或近2年有1–2篇中文核心期刊文章
4	担任中华医学会呼吸分会学组委员，或有SCI论文/全国会议讲者经历
5	国家级学会常委/组长，牵头指南/共识，国际会议发言
治疗理念	
1	仍以ICS单药或短效药为主，排斥三联/双支扩
2	知晓但仅用于极重度患者，观念保守
3	在部分合适患者中尝试使用，但依赖传统阶梯疗法
4	主动评估症状/急性加重风险，按指南推荐使用三联/双支扩
5	积极倡导“早优化”理念，在科室/区域推广GSK治疗路径
产品经验      
1	未处方过任何GSK呼吸产品
2	仅偶尔处方舒利迭（老产品），不了解新药
3	熟悉舒利迭，开始尝试欧乐欣或全再乐（<6个月）
4	三种产品均有稳定处方，能根据患者特征选择合适剂型
5	高频处方全再乐/欧乐欣，作为一线推荐，并指导下级医生使用
患者管理  
1	不进行吸入装置教学，无随访机制
2	偶尔演示，但无系统流程
3	有护士/药师协助教学，部分患者接受培训
4	常规进行吸入技术评估与纠正，有复诊提醒机制
5	建立慢病管理路径（如COPD门诊），使用数字化工具追踪依从性    
区域带教      
1	仅服务本院患者，无外部影响
2	偶尔参与县级医院会诊
3	定期参与医联体培训，带教住院医
4	主办市级继续教育项目，指导多家医院规范用药
5	牵头区域COPD/哮喘联盟，主导多中心协作，积极推荐GSK方案
